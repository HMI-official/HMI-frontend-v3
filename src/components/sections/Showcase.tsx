...
import { SLIDER_CARD_NUMBER, SLIDER_CARD_WIDTH } from "../../constants";
...
const slide = keyframes`
  ...
  100% {
    transform: translate(
      calc(
        -${SLIDER_CARD_WIDTH}* ${SLIDER_CARD_NUMBER} - 2rem * ${SLIDER_CARD_NUMBER}
      ),
      0px
    );
  }
...
const NftItem: FC<INftItem> = (props) => {
  let play = () => (props.passRef.current.style.animationPlayState = "running");
  let pause = () => (props.passRef.current.style.animationPlayState = "paused");

  return (
    <ImgContainer onMouseOver={pause} onMouseOut={play}>
      <img width={500} height={400} src={props.img} alt="The Weirdos" />
      <Details>
        <div>
        ...
        <div>
          ...
          <Price>
            ...
            <h1>{Number(props.price).toFixed(1)}</h1>
          </Price>
        </div>
      </Details>
    </ImgContainer>
